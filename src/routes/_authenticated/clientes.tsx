    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cadastro, edição e histórico</p>
        </div>
        <Button onClick={openNew} className="rounded-lg"><Plus className="size-4 mr-2" /> Novo cliente</Button>
      </div>
          ...
          {filtered.map((c) => (
            <Card key={c.id} className="p-5 rounded-xl space-y-3">
              ...
              <div className="flex gap-2 pt-1">
                <Button size="sm" variant="outline" className="flex-1 rounded-lg" onClick={() => openEdit(c)}>
